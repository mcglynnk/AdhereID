...
def seleckbest_on_categorical_feats(X, y):
    cols = X.columns
    kbest_selector = SelectKBest(chi2, k=4)
    X_new = kbest_selector.fit_transform(X, y)
    print(pd.DataFrame(kbest_selector.inverse_transform(X_new), columns=cols))
    return X_new

seleckbest_on_categorical_feats(X_processed.iloc[:,11:], y)